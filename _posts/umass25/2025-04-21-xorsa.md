...
But there's something unusual: the script leaks $$\text{partial } p \oplus q: \text{hex}((p \oplus q) \gg (\text{bits} // 2 - \text{extra}))$$. This means we're getting a significant portion of the XOR of $$p$$ and $$q$$, specifically the most significant $$\text{bits}/2 + \text{extra}$$ bits (where $$\text{extra} = 75$$).
...
$$\text{min\_prod} = (p_{\text{high\_new}} \ll \text{shift}) \cdot (q_{\text{high\_new}} \ll \text{shift})$$

$$\text{max\_prod} = ((p_{\text{high\_new}} \ll \text{shift}) + (2^{\text{shift}} - 1)) \cdot ((q_{\text{high\_new}} \ll \text{shift}) + (2^{\text{shift}} - 1))$$

$$\text{if } \text{min\_prod} \leq n \leq \text{max\_prod}: \quad \text{This path might lead to a solution}$$

This check is crucial:
- $$\text{min\_prod}$$ represents the product if all remaining bits are 0
- $$\text{max\_prod}$$ represents the product if all remaining bits are 1
- If $$n$$ is outside these bounds, this path cannot lead to a solution
...
2. **Min/Max bounds**: This is the key insight. For any partially constructed $$p_{\text{high}}$$ and $$q_{\text{high}}$$:
   ...
   - If all remaining bits are 1, the product would be at most:
     $$((p_{\text{high}} \ll \text{shift}) + (2^{\text{shift}} - 1)) \cdot ((q_{\text{high}} \ll \text{shift}) + (2^{\text{shift}} - 1))$$
