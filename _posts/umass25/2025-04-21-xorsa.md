...
But there's something unusual: the script leaks $$\boldsymbol{\text{partial } p \oplus q: \text{hex}((p \oplus q) \gg (\text{bits} // 2 - \text{extra}))}$$. This means we're getting a significant portion of the XOR of $$p$$ and $$q$$, specifically the most significant $$\text{bits}/2 + \text{extra}$$ bits (where $$\text{extra} = 75$$).
